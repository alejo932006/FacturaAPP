...
public class FacturaGUI extends JFrame implements ClienteSeleccionListener {
    ...
    private JTextField txtNombre, txtCedula, txtDireccion, txtCantidad, txtPrecio;
    ...
    private void agregarProducto() {
        ...
        if (factura == null) {
            // (El resto de la lógica para crear la factura no cambia)
            ...
            if (!nombre.isEmpty() && !cedula.isEmpty() && !cedula.equals("0") && !ClienteStorage.clienteExiste(cedula)) {
                ...
                if (respuesta == JOptionPane.YES_OPTION) {
                    ...
                    setDatosCliente(nombre, cedula, direccion);
                } else {
                    clienteParaFactura = new Cliente("Cliente Ocasional", "0", direccion);
                }
            } else {
                clienteParaFactura = new Cliente(nombre.isEmpty() ? "Cliente Ocasional" : nombre, cedula.isEmpty() ? "0" : cedula, direccion);
            }
    ...
    public void setDatosCliente(String nombre, String cedula, String direccion) {
    ...
    private File registrarFacturaEnArchivo() {
        ...
        try {
            ...
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(archivoFactura))) {
                writer.write(factura.generarTextoFactura());
            }
